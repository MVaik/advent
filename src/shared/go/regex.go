...
var NUMBER_REGEX_NON_NEGATIVE = regexp.MustCompile(`\d+`)
...
func MatchPositiveIntsAndParse(input string) []int {
	matchedNums := NUMBER_REGEX_NON_NEGATIVE.FindAllString(input, -1)
	parsedInts := make([]int, 0, len(matchedNums))
